...
from PyQt5.QtWidgets import QTextEdit, QInputDialog, QHBoxLayout, QVBoxLayout, QFormLayout
...
notes_win = QWidget() # create a widget
notes_win.setWindowTitle('Smart Notes')
notes_win.resize(1000, 700) # size of widet is 1000x700
...
field_text.setPlaceholderText('Enter text of the note...')
...
def add_note():
    note_name, ok = QInputDialog.getText(notes_win, 'Add note', 'Note_name:')
    if ok and note_name != '':
        notes[note_name] = {'text' : '', 'tags' : []}
        list_notes.addItem(note_name)
        list_tags.addItem(note_name)
        list_tags.addItems(notes[note_name]['tags'])
        print(notes)

#sort_keys = mengurutkan list bentuk alphabet
...
def show_note():
    ...
    list_tags.clear() # INI DIGUNAKAN SUPAYA TAG YANG TAMPIL HANYA TAG DARI  NOTEYANG DIPILIH
    list_tags.addItems(notes[key]["tags"]) # INI GUNANYA MENAMPILKAN SESUATU BERDASARKAN TAG
def del_note():
    if list_notes.selectedItems():
...
palette = notes_win.palette()
palette.setColor(QPalette.Window, QColor("grey"))
palette.setColor(QPalette.WindowText, QColor("darkyellow"))
notes_win.setPalette(palette)
